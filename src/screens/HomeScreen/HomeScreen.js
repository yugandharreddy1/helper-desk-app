...
export default function HomeScreen(props) {
  ...
  useEffect(() => {
    const activitiesArr = [];
    const activitiesRef = firebase.firestore().collection("Helper-app-data");
    activitiesRef
      .doc("Activites")
      .get()
      .then((firestoreDocument) => {
        if (!firestoreDocument.exists) {
          alert("Activites does not exist anymore.");
          return;
        }
        const { key, value } = firestoreDocument.data();
        activitiesArr.push({
    ...
    activitiesRef
      ...
      .then((firestoreDocument) => {
        if (!firestoreDocument.exists) {
          alert("Times does not exist anymore.");
          return;
        }
        console.log(firestoreDocument.data());
      })
  ...
  return (
    <NativeBaseProvider>
      <Container alignItems="center">
        ...
        <Fab
          onPress={(() => setShowModal(true), getLatestEvents())}
          placement="top-right"
              ...
              <DataTable>
                ...
                {tasks.map((item) => (
                  <DataTable.Row>
                    ...
                    <DataTable.Cell>{item.booked}</DataTable.Cell>
                    <DataTable.Cell>{item.bookedUser}</DataTable.Cell>
                  </DataTable.Row>
                ))}
              </DataTable>
            </Modal.Body>
